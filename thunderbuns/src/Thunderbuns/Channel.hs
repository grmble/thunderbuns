...
class Monad m =>
  ...
  messagesBefore :: V Channel -> V OrderedUUID -> m [Msg] -- ^ messages in the channel before message pk
...
instance (HasDbConnection r, HasEventChannel r, HasLogger r) =>
  ...
  messages c =
    runDB $ do
      ...
      for
        (reverse rows)
        (extractRow
           (Msg <$> pure (unV c) <*> fmap orderedUUID extract <*> extract <*>
            extract))
  messagesBefore c pk =
    runDB $ do
      let cql =
            "select created, user, msg from tb.msg where channel=? and created<? limit 10"
      rows <-
        executeQuery
          Quorum
          cql
          [TextValue (channelName (unV c)), TimeUUIDValue (toUUID $ unV pk)]
      for
        (reverse rows)
        (extractRow
           (Msg <$> pure (unV c) <*> fmap orderedUUID extract <*> extract <*>
  ...
  addMessage vm = do
    ...
    runDB $
      execute
        ...
        [TextValue c, TimeUUIDValue (toUUID k), TextValue u, TextValue m]
  ...
  mkMsg c u s =
    ...
    maybe
      ...
      (\uuid -> pure $ Msg c (orderedUUID uuid) u s)
