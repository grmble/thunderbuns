module Thunderbuns.Utils (supervise, microSeconds) where

import Control.Concurrent (threadDelay)
import System.Random (getStdRandom, randomR)
import Thunderbuns.Tlude
import UnliftIO.Exception (SomeException, catch, finally)
...
supervise :: forall r m. (Bunyan r m, MonadUnliftIO m) => Text -> Int -> Int -> m () -> m ()
supervise name minmu maxmu action =
  ...
  where
    ...
    ignoreException ex = logRecord ERROR (someException ex) "caught exception in supervise"
    --
