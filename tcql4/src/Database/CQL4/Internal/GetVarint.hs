...
unsignedVarint = do
  ...
  G.label "unsignedVarint" $
    G.isolate n $ go n (fromIntegral $ b .&. firstByteValueMask n)
  where
    go 0 !acc = pure acc
    go n !acc = do
      b <- G.getWord8
...
firstByteValueMask n = 0xFF `shiftR` n

decodeZigZag64 :: Word64 -> Int64
decodeZigZag64 w = fromIntegral $ (w `shiftR` 1) `xor` negate (w .&. 1)

encodeZigZag64 :: Int64 -> Word64
encodeZigZag64 i = fromIntegral $ (i `shiftL` 1) `xor` (i `shiftR` 63)
