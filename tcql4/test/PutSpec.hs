    ...
    it "encode flags" $
      P.runPut
        (frameHeader
           ...
           OpError
           0x4eadbeef) `shouldBe`
      "\x04\x09\0\0\0\x41\0\x4e\xad\xbe\xef"
    it "encode stream id/length" $
      P.runPut (frameHeader RequestFrame [] 0x4eadbeef OpError 0x41) `shouldBe`
      "\x04\0\x4e\xad\xbe\xef\0\0\0\0\x41"
    it "encode op code" $
      P.runPut (frameHeader RequestFrame [] 0x41 OpAuthSuccess 0x4eadbeef) `shouldBe`
      "\x04\0\0\0\0\x41\x10\x4e\xad\xbe\xef"
