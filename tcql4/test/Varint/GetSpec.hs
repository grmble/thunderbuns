...
spec = do
  ...
  describe "numberOfExtrabytesToRead" $ do
    ...
    it "can handle two extra bytes" $
      numberOfExtraBytesToRead 0xc0 `shouldBe` 2

  describe "firstByteValueMask" $ do
    it "can handle 0 extra bytes" $
      firstByteValueMask 0 `shouldBe` 0xFF
    it "can hancle single extra byte" $
      firstByteValueMask 0x1 `shouldBe` 0x7F
    it "can hancle two extra bytes" $
      firstByteValueMask 0x2 `shouldBe` 0x3F
    it "can hancle 8 extra bytes" $
      firstByteValueMask 0x8 `shouldBe` 0x00


  describe "unsignedVarint" $ do
    it "can handle 0" $
      runGet unsignedVarint "\x00" `shouldBe` Right 0
  ...
  describe "zigZag64" $
    it "decode . encode == id" $ property $
      \x -> decodeZigZag64 (encodeZigZag64 x) == (x :: Int64)

  describe "varint" $ do
    it "can decode 0" $
      runGet varint "\x00" `shouldBe` Right 0
    it "can decode -1" $
      runGet varint "\x01" `shouldBe` Right (-1)
    it "can decode 1" $
      runGet varint "\x02" `shouldBe` Right 1
    it "can decode 512" $
      runGet varint "\x84\x00" `shouldBe` Right 512
    it "can decode -512" $
      runGet varint "\x83\xff" `shouldBe` Right (-512)
